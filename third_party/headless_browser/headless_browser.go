...
type Config struct {
	...
	Leakless bool // Whether rod may launch its auxiliary leakless process
}
...
func newDefaultConfig() *Config {
	return &Config{
		...
		StealthJS:     true,
		Trace:         false,
		Leakless:      leaklessDefault(runtime.GOOS),
	}
}

func leaklessDefault(goos string) bool {
	// Windows 服务由 Browser.Close 正常回收，避免执行会被 Defender 拦截的 leakless.exe。
	return goos != "windows"
}
...
// WithLeakless controls rod's auxiliary leakless process.
